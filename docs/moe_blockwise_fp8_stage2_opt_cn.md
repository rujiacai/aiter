...
| fabric 读带宽占用 | ~~gemm1 93.8%、gemm2 89.1% 的实测上限（2662 GB/s）~~ **此行已作废，见 §10**：2662 GB/s 是 rocprof-compute 的配置常量，实测纯读屋顶 ~4.4 TB/s，gemm1 只到 66% |
| 生效方式 | 默认生效，无需 env |
...
## 10. 修正 §8：屋顶不是 2662 GB/s，瓶颈是请求数不是字节数

日期：2026-09-11。机器同上（MI308X / gfx942 / 80 CU）。

§8 的结论「gemm1 已经贴着 fabric 读上限 93.8%，唯一提速方式是少读字节」**建立在一个错误的
上限值上**，因此「已经没有空间了」这个停止判据是不成立的。下面是重测。

### 10.1 实测读屋顶 ≈ 4.4 TB/s，不是 2662 GB/s

2662 GB/s 是 rocprof-compute 给这块卡配的 **L2-Fabric peak 常量**，不是实测能力。
用一个独立的纯读 kernel（`buffer_load_dwordx4`，无写、无 MFMA、无 LDS、无 barrier，
工作集 1 GB 已远超 256 MB MALL）扫「并发 WG 数 × 每线程在飞 load 数」：

| blocks | WG/CU | u1 | u2 | u4 | u8 | u16 | u32 |
| ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| 160 | 2.0 | 1416 | 2446 | 3909 | **4440** | 3825 | 3310 |
| 240 | 3.0 | 2008 | 3339 | **4467** | 3260 | 3235 | 4039 |
| 320 | 4.0 | 2559 | **4093** | 4286 | 3182 | 3368 | 3804 |
| 480 | 6.0 | 3471 | **4531** | 3664 | 3821 | 3792 | 3637 |
| 640 | 8.0 | **3953** | 3472 | 3796 | 3847 | 3800 | 3713 |
| 960 | 12.0 | 3222 | **4032** | 3896 | 3806 | 3805 | 3731 |

（单位 GB/s。列 = `#pragma unroll` 出来的、在第一条 `s_waitcnt` 之前背靠背发出的 dwordx4 条数。）

两个结论：

1. **可达读带宽 ~4.4 TB/s**，是 §8 所用 2662 的 1.65 倍。
2. **带宽由每 wave 在飞的 load 数决定，不由 occupancy 决定**：2 WG/CU 只要有 8 条在飞就能跑到
   4440 GB/s，而 8 WG/CU 每线程只发 1 条时只有 3953。低 occupancy 不等于低带宽。

### 10.2 真实 HBM 流量（rocprofv3 `--pmc FETCH_SIZE`，bs128 layer3）

| | 请求字节（发出） | HBM 实取 | L2 吸收 | 理论最小 | 时间 | **请求速率** | **HBM 速率** |
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| gemm1 | 604 MB | **448.6 MB** | 155 MB | 402.7 MB | 155.4 µs | **3.89 TB/s** | **2.89 TB/s** |
| gemm2 | 302 MB | **214.4 MB** | 88 MB | 201.3 MB | 82.8 µs | **3.65 TB/s** | **2.59 TB/s** |

「请求字节」= 真实 M-block 数 24 × 每 M-block 要流过的整片权重（W1 25.17 MB / W2 12.58 MB）。
`tile_m=16` 下热专家跨 2 个 M-block，所以 24 个 block 只对应 16 个专家的 402.7 MB 唯一权重。

### 10.3 重新定位瓶颈：**request-throughput bound，不是 HBM bound**

把 10.1 和 10.2 对起来看：

- HBM 速率 2.89 TB/s，只有屋顶 4.4 的 **66%** —— 看起来还差很远；
- 但**请求速率已经是 3.89 TB/s = 屋顶的 87%**，且 gemm1 每线程每拍恰好发 4 条 dwordx4，
  对应上表 3 WG/CU 的 u4 格子（4467 GB/s）—— **已经贴着那一格**。

也就是说：**kernel 的发射端已经接近饱和，而其中 26% 的请求（155 MB）是被 L2 吃掉的权重重读。**
HBM 端看起来「空」，不是因为没能力打满，而是因为发射槽被冗余请求占着。

这把优化方向整个翻过来了：

> §8 说「时间 ≈ 字节数 / 2.5 TB/s，唯一提速方式是少读**字节**」。
> 正确的说法是「时间 ≈ **请求数** / ~4 TB/s，提速方式是少发**请求**」。
> 而 §4 里那些「减少权重重读」的尝试之所以失败，不是因为方向错，是因为它们
> （`tile_m=32`）在减请求的同时把 occupancy 砍了一半。

按请求数折算的地板：gemm1 402.7 MB / 3.89 TB/s = **103 µs**（当前 155.4），
gemm2 201.3 / 3.65 = **55 µs**（当前 82.8）。加上 sorting+quant 约 22 µs，
e2e ≈ **180 µs**，对应 603.98 MB / 180 µs = **3.36 TB/s** —— 3 TB/s 的目标是可达的，
但**只能靠消除权重重读拿到**，不能靠继续调发射端。

### 10.4 本轮的负结果

三个都是在上述认识之前试的，记录下来避免重走：

| 尝试 | 结果 | 为什么 |
| --- | --- | --- |
| **B 预取深度 1 → 2**（`AITER_BLKFP8_B_PREFETCH`，代码已落地，默认仍为 1） | gemm1 156.0 → **163.0**（+4.5%），gemm2 不变 | 多带一个 B tile 正好 +32 VGPR：`20+132=152` 允许 3 waves/SIMD，`52+132=184` 只允许 2。用 1/3 的 occupancy 换了一倍在飞 load，而 depth 1 的 4 条/线程本来就已经在 10.1 表的最优格附近，没有需求 |
| `AITER_BLKFP8_FMA_DEPTH` 0 / 1 / 2 / 4 重扫 | gemm1 155.3~155.9、gemm2 82.8~83.0，**全在噪声内** | AGPR 恒为 132，FMA pipe 的 pending partial 没有大到能改变分配档位 |
| 指望「提高 fabric 占用率」本身 | 不是可优化量 | 见 10.3：HBM 端的空闲是冗余请求的**症状**，不是原因 |

寄存器实测（`rocprofv3 --kernel-trace`，bs128）：

| | VGPR | AGPR | 总计 | waves/SIMD | Scratch |
| --- | ---: | ---: | ---: | ---: | ---: |
| gemm1 depth1（当前默认） | 20 | 132 | **152** | 3 | 0 |
| gemm1 depth2 | 52 | 132 | 184 | 2 | 0 |
| gemm2 | 16 | 128 | **144** | 3 | 0 |

gemm1 的 132 个 AGPR 里**只有 16 个是真累加器**（`num_acc_n=2 × m_repeat=1 × 2(gate+up) × 4 f32`），
其余 116 个是循环携带状态被分配器放进了 AGPR。要跨过 128 拿 4 waves/SIMD 得砍掉 24 个，
FMA pipe 那条路已证明不够（10.4 第二行）。

### 10.5 下一步：唯一值得做的是消除权重重读

目标是让**同一个专家的多个 M-block 复用同一次权重流**，而不是各自重新发一遍请求。
关键是要**在不掉 occupancy 的前提下**做到 —— §4 的 `tile_m=32` 失败正是因为没做到这点。

候选做法，按推荐顺序：

1. **WG 内沿 M 方向循环同一专家的多个 M-block**（persistent-expert 调度）。
   grid 从 `(n_tile, m_block)` 改成 `(n_tile, expert)`，WG 内按 `sorted_expert_ids` 循环该专家的
   M-block，B tile 留在寄存器里跨 M-block 复用。请求数 604 → 403 MB。
   寄存器代价只有多出来的累加器（每多一个 M-block +16 AGPR），**比 `tile_m=32` 便宜得多**，
   因为 X 的 LDS tile 和 B tile 都不用变大。
   这正是手写 ASM 1stage 用 persistent TG 池做的事，也解释了它在 bs128 上快 6.3%。
2. 若 1 的动态循环次数不好做，退一步：**只融合「同一专家恰好 2 个 M-block」的情形**
   （bs128 下 blocks/expert=1.5，约一半专家命中），拿一半收益但实现简单得多。
3. a2 量化融进 stage1 epilogue（§8 已列，7.5 µs，与本节正交，仍然值得做）。

### 10.6 复现

```bash
# 纯读屋顶（10.1）
hipcc -O3 --offload-arch=gfx942 bwtest2.hip -o bwtest2 && ./bwtest2

# 真实 HBM 流量（10.2）
AITER_FLYDSL_BLKFP8=1 AITER_BYPASS_TUNE_CONFIG=1 \
  rocprofv3 --pmc FETCH_SIZE --kernel-include-regex moe_gemm \
  -d /tmp/prof -o p --output-format csv \
  -- python op_tests/test_fmoe_vllm_dump.py -b 128 -l 3
# FETCH_SIZE 单位是 KB，除以 1024 得 MB；注意除以 dispatch 次数

# 寄存器数（10.4）
... rocprofv3 --kernel-trace --kernel-include-regex moe_gemm ...
# 取 VGPR_Count / Accum_VGPR_Count / Scratch_Size

# B 预取深度 A/B（务必每格独立 cache 目录）
FLYDSL_RUNTIME_CACHE_DIR=/tmp/fdc_d2 AITER_BLKFP8_B_PREFETCH=2 \
  AITER_LOG_MORE=1 AITER_FLYDSL_BLKFP8=1 AITER_BYPASS_TUNE_CONFIG=1 \
  python op_tests/test_fmoe_vllm_dump.py -b 128 -l 3
```

---

## 11. tile_m × tile_n 交叉：§10.5 的 run-fusion 被证伪，但换来一个可落地的配置

日期：2026-09-11（承接 §10）。

### 11.1 先证伪 §10.5 的第 1 条

§10.5 推荐「persistent-expert 调度」（grid 从 `(n_tile, m_block)` 改成 `(n_tile, expert)`，
WG 内循环同一专家的多个 M-block 复用 B tile）。**先别写 —— 它和 `tile_m=32` 是等价的。**

bs128 layer3 的 24 个 M-block 分布在 16 个专家上，融合后剩 16 个 leader WG，请求
16 × 25.17 = 402.7 MB；`tile_m=32` 同样是 16 个 M-block、同样 402.7 MB。两者都要求两个
M-block 的累加器同时存活才能复用 B tile，即 `m_repeat=2` —— 寄存器代价一模一样，而寄存器
是静态分配的，只有 1 个 block 的专家也照付。唯一的差别是 sorting 的 padding，不在关键路径上。

所以先用一行改动复测 `tile_m=32`（bs128 layer3）：

| | tile_m=16 | tile_m=32 |
| --- | ---: | ---: |
| gemm1 | 155.6 µs | **170.6**（+9.6%） |
| gemm1 HBM 实取 | 448.5 MB | **391.0 MB**（−12.8%） |
| gemm1 发出请求 | 604 MB | **403 MB**（−33%） |
| **请求速率** | **3.88 TB/s** | **2.36 TB/s**（−39%） |
| gemm1 寄存器 | 20+132=152 → **3 waves/SIMD** | 60+132=192 → **2 waves/SIMD** |

请求数按 §10.3 的预测降了 33%，**但请求速率崩了 39%** —— occupancy 从 3 掉到 2 吃掉的比省下的多。
run-fusion 会撞完全同一堵墙，不必实现。

### 11.2 缺的那一半：`tile_m=32` 必须配窄 N tile

§4 只在 `tile_m=16` 下扫过 `tile_n`，交叉格是空的。补上（bs128 layer3）：

| cell | gemm1 | gemm2 | e2e | gemm1 寄存器 | gemm2 寄存器 |
| --- | ---: | ---: | ---: | --- | --- |
| m16 n1=128 n2=256（原默认） | 155.6 | 82.7 | 261.2 | 152 → 3 waves | 144 → 3 waves |
| m16 n1=64 | 170.5 | 83.0 | 276.3 | 96 → 5 waves | 144 → 3 waves |
| m32 n1=64 n2=256 | 152.0 | 83.4 | 258.1 | **128 → 4 waves** | 184 → 2 waves |
| **m32 n1=64 n2=128** | **152.3** | **78.9** | **255.0** | **128 → 4 waves** | **120 → 4 waves** |
| m32 n1=128 n2=128 | 170.5 | 78.7 | 272.6 | 192 → 2 waves | 120 → 4 waves |

`num_acc_n = tile_n/4/16`，所以窄 N tile 直接把循环携带的 B tile 减半，正好把
`tile_m=32` 多出来的累加器付掉。**`_tile_n2=128` 在 `tile_m=16` 下比 256 慢 18 µs（§4），
在 `tile_m=32` 下却稳定快 4 µs** —— 这个反转只有交叉扫才看得到。

### 11.3 `waves_per_eu` 救不了宽 N tile（负结果）

m32/n1=128 那格同时具备低流量和每 wave 4 条在飞 load，只卡在 192 寄存器。
用 `waves_per_eu` 直接给分配器下预算：

| cell | gemm1 | 寄存器 | **Scratch** |
| --- | ---: | --- | ---: |
| m32 n1=128 **wpe1=3** | **297.3** | 40+128=168 → 3 waves | **116** |
| m32 n1=128 **wpe1=4** | **820.5** | 128+0=128 → 4 waves | **376** |
| m32 n1=64 wpe 3/3 | 151.65 | 128 → 4 waves | **0** |

分配器确实压到了预算内，**但靠溢出到 scratch**，gemm1 2× / 5× 地崩。
这个 kernel 在宽 N tile + `tile_m=32` 下真的需要 192 个寄存器，
**要拿到「低流量 + 4 条在飞」必须在 kernel 源码层面减活跃状态，编译期旋钮已经用尽。**

### 11.4 落地的配置与跨 bs 结果

`aiter/fused_moe.py` 的 fp8blk 分支：

```python
_tile_m  = 16 if token < 512 else 32               # 原来是 <= 512
_tile_n1 = 64 if (token <= 8 or _tile_m == 32) else 128
_tile_n2 = 128 if _tile_m == 32 else 256
_wpe     = 3 if _tile_m == 32 else 2               # kn1/kn2 的 _w 后缀
```

layer3，无任何 env 变量，device avg：

| bs | rows | e2e 前 | e2e 后 | Δ | zero_rows | relL2 |
| ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| 16 | 64 | 148.8 | 148.8 | 0（未触碰） | 0 | 0.0243 |
| 64 | 256 | 203.8 | 203.9 | 0（未触碰） | 0 | 0.0242 |
| 128 | 512 | 260.8 | **256.7** | **−1.6%** | 0 | 0.0242 |
| 192 | 768 | 303.3 | **296.0** | **−2.4%** | 0 | 0.0241 |
| 224 | 896 | 342.2 | **323.1** | **−5.6%** | 0 | 0.0242 |

bs128 的重复测量落在 253.0~256.7，取 **−2±1%**。
bs192/224 本来就走 `tile_m=32`，它们的收益**只来自 n1/n2/wpe 的重调**，
是这套配对最干净的证据。bs16/64 由阈值挡在外面，逐位未变。
...
| 杠杆 | 状态 |
| --- | --- |
| 流量（发出的请求） | `tile_m=32` 已拿到，604 → 403 MB |
| occupancy | 窄 N tile 已拿到，两个 stage 都到 4 waves/SIMD |
| 每 wave 在飞 load 数 | **没拿到** —— 窄 N tile 把它从 4 条砍到 2 条 |

所以 bs128 的请求速率还是 2.6 TB/s 量级，离 §10.1 的 ~4.4 TB/s 屋顶很远，
e2e 2.37 TB/s 离 3 TB/s 也还有距离。**剩下的全部收益都在第三行**，
而 §11.3 证明了它不能靠编译期旋钮拿，只能减 kernel 的活跃状态：

1. **B tile 直接 gmem→LDS（`global_load_lds` / async copy）**。B 目前占着循环携带状态的大头
   （n1=128 时 carried + in-flight 两份 ≈ 68 个寄存器），直接落 LDS 可以把预取深度和寄存器
   压力解耦，这样 m32 + n1=128 就付得起了。§4 记过「stage2 `use_async_copy` 该 kernel family
   未实现」—— 这是现在最值得实现的一条。
2. 退一步：ping/pong 只留一份 B 缓冲，省掉 in-flight 那 34 个。
3. a2 量化融进 stage1 epilogue（§8 已列，7.5 µs，与上面正交）。
