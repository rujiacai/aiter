    ...
    *,
    ...
    if True:
        ...
        def moe_gemm2(
            ...
            # The e8m0 microscale preshuffle groups K-scale columns by 4(klane)*2(kpack)=8,
            # i.e. K by 256. `shuffle_scale` zero-pads the scale K-columns up to a
            # multiple of 8 for non-256-aligned inter_dim (e.g. 384 -> 512). Round the
            # layout's K up to match so stride_n0 (= c_k1 * stride_k0) is consistent
            # with the padded scale tensor. No-op for 256-aligned inter_dim.
            _scale_k_round = 32 * 4 * 2  # scale_block * k_lane * k_pack = 256
            _inter_dim_scale = (
                (inter_dim + _scale_k_round - 1) // _scale_k_round
            ) * _scale_k_round
            c_k_orig = arith.constant(_inter_dim_scale, index=True)
            layout_a_scale = make_preshuffle_scale_layout(
            ...
            if const_expr(not is_f16_b):
                ...
                # The scale tensor is K-padded to `_inter_dim_scale` (inter_dim
                ...
                kblk_w = arith.constant(_inter_dim_scale // 32, index=True)  # padded K/32
