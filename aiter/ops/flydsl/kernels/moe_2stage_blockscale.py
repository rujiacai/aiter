...
# buffer_ops / vector live in aiter's kernel shim on this branch, not in flydsl.expr.
...
# How many MFMAs to keep in flight before applying a queued post-MFMA scale FMA.
# The blockwise path chains a whole K block into a zero accumulator and folds the
...
def compile_moe_gemm1(
    *,
    ...
    # This module only serves the blockwise-fp8 family. "fp8" (per-row/per-token
    # f32 scales) is kept alongside "fp8_blk" because it shares the whole pipeline
    # and is the control group when debugging a blockwise scale-indexing problem.
    _valid_dtypes = (
        "fp8",
        "fp8_blk",
    )
    ...
    if True:
        ...
        def moe_gemm1(
            ...
            # Weights are always (16,16)-preshuffled fp8 on this path.
            w_elem_bytes = elem_bytes
            kpack_bytes = 16
