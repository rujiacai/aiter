...
def fused_moe(
    ...
    # smoothquant pre-scaling of the down-projection input, folded into stage1's
    # activation: act *= smooth_scale[expert]. [expert(local_expert:EP), inter_dim]
    ...
    if (
        ...
        if smooth_scale is not None:
            raise NotImplementedError(
                "smooth_scale is not supported by the shared-expert (fhmoe) path"
            )
...
def _check_swiglu_limit_supported(metadata, swiglu_limit, activation):
    ...
    Only the FlyDSL and Opus stage1 kernels read `swiglu_limit`; the asm 1-stage /
    2-stage, CK and cktile kernels have no operand for it, so without this a
    caller asking for e.g. the DeepSeek-V4 limit of 10.0 would get an unclamped
    result with no diagnostic.
    """
    if not swiglu_limit:
        return
    stage1_func = getattr(metadata.stage1, "func", metadata.stage1)
    ...
    raise NotImplementedError(
        ...
        f"AITER_FLYDSL_BLKFP8=1 to route to the FlyDSL kernel, which bakes the "
        f"clamp in; if a tuned CSV row still forces the asm 1-stage kernel, add "
        f"AITER_BYPASS_TUNE_CONFIG=1."
    )
