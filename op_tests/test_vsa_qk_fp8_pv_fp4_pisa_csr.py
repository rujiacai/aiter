...
Checks:
  ...
  3. ρ>0 with ≤8 approx blocks/row — the kernel folds those into the exact
     loop, so it must stay as accurate as running them as exact.
...
def test_promote_below_threshold_stays_exact_accurate():
    """Rows with ≤ APX_PROMOTE_EXACT approx blocks never build centroids.

    The kernel folds them into the exact FP8-QK / FP4-PV loop, so the split
    launch has to land on the same accuracy as sending every block down the
    exact stream.  Scoring it against a reference that runs those blocks as
    centroids instead measures the reference's approximation, not the
    kernel's, and reads far lower.
    """
    n_approx = APX_PROMOTE_EXACT - 1
    data = make_synthetic_data(
        seed=1, num_q_blks=16, B=1, H=4, sparsity=12 / 16, dense_frac=0.0
    )
    B, T, num_q_blks, H = data["B"], data["T"], data["num_q_blks"], data["H"]
    exact_idx, exact_num, approx_idx, approx_num = _split_padded(
        data["q2k_idx"], data["q2k_num"], n_approx
    )
    assert 0 < int(approx_num.max().item()) <= APX_PROMOTE_EXACT
    ...
    assert not plan.need_stats, "geometry must exercise the promote path"

    split_out, _ = vsa_qk_fp8_pv_fp4_pisa_csr_dropB(
        q=data["q"], k=data["k"], v=data["v"],
        qs=data["qs"], ks=data["ks"], vs=data["vs"], vbs=data["vbs"],
        plan=plan, B=B, T=T, num_q_blks=num_q_blks,
    )

    # Reference: the union of both streams, every block exact, in BF16.
    q_bf, k_bf, v_bf = _dequant_bh(data)
    width = exact_idx.shape[-1]
    empty_idx = torch.full_like(data["q2k_idx"].view(-1, width), -1)
    empty_num = torch.zeros(
    ...
    cos_split = _cosine(split_out, golden.view_as(split_out))

    all_exact_col, all_exact_meta = padded_to_csr(
        data["q2k_idx"], data["q2k_num"]
    )
    all_exact_out, _ = attention_pisa_csr(
        q=data["q"], k=data["k"], v=data["v"],
        qs=data["qs"], ks=data["ks"], vs=data["vs"], vbs=data["vbs"],
        exact_index=all_exact_col, exact_meta=all_exact_meta,
        B=B, T=T, num_q_blks=num_q_blks,
    )
    cos_all_exact = _cosine(all_exact_out, golden.view_as(all_exact_out))
    print(
        f"promote path ({n_approx} approx/row) cosine vs all-exact BF16: "
        f"{cos_split:.6f}  (all-exact route: {cos_all_exact:.6f})"
    )
    assert cos_split > cos_all_exact - 0.002, (
        f"promoted blocks lost accuracy: {cos_split} vs {cos_all_exact}"
    )
...
if __name__ == "__main__":
    ...
    test_promote_below_threshold_stays_exact_accurate()
